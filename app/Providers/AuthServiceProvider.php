...
class AuthServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        Gate::define('seller_access', function ($user) {
            return in_array($user->role_id, [1, 2]);
        });
        Gate::define('seller_create', function ($user) {
            return in_array($user->role_id, [1, 2]);
        });
        Gate::define('seller_edit', function ($user) {
            return in_array($user->role_id, [1, 2]);
        });
        Gate::define('seller_view', function ($user) {
            return in_array($user->role_id, [1, 2]);
        });
        Gate::define('seller_delete', function ($user) {
            return in_array($user->role_id, [1, 2]);
        });

        // Auth gates for: Inventario
        Gate::define('stock_access', function ($user) {
            return in_array($user->role_id, [1]);
        });
        Gate::define('stock_create', function ($user) {
            return in_array($user->role_id, [1]);
        });
